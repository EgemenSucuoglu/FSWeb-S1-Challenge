...
const newArray=[];
function fenomenEkle(fenomenlerBolumu,sayilar,profil,takipci,gonderiler,uygulamaAdi) {
  const newArray=[...fenomenlerBolumu];
  newArray.push({
    "number": sayilar,
    "profile": profil,
    "followers": takipci,
    "posts": gonderiler,
    "platform": uygulamaAdi,
  }
  )
  return newArray; 
}
...
function enFenomenler(array) {
  const enFenomenlerNew = [];
  for (let i=0, i<array.length, i++) {
    if (let array[i].followers >= 100000000 ){
    enFenomenlerNew.push(array[i].profile);
  }
 }
  return enFenomenlerNew
}
